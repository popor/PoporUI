...
- (void)playOneLoopFinshBlock:(void(^)(void))finshBlock {
    YLImageView * gifIV = self;
    gifIV.autoPlay = YES;
    gifIV.loopCountdown = 1;
    [gifIV startAnimating];
    
    
    gifIV.loopCountdown = 1;
    gifIV.currentFrameIndex = 0;
    YLGIFImage *gifImage = (YLGIFImage*)gifIV.image;
    
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)((gifImage.duration + 0.05)* NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        if (finshBlock) {
            finshBlock();
        }
